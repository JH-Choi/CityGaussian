...
from utils.camera_utils import loadCam, loadCam2
from utils.general_utils import safe_state, parse_cfg, PILtoTorch
import uuid
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, refilter_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    while iteration <= opt.iterations:
        ...
        v_cam = viewpoint_stack.pop(idx)
        viewpoint_cam = loadCam2(dataset, idx, v_cam, 1)
        ...
        # Loss
        start = time.time()
        gt_image = PILtoTorch(Image.open(viewpoint_cam.image_path), (viewpoint_cam.image_width, viewpoint_cam.image_height))
        ...
        with torch.no_grad():
            ...
            tmp_path = os.path.join(scene.model_path, 'temp')
